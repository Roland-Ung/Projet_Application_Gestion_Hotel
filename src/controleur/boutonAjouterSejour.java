		...
		try {
			...
			for (Client c : hotel.ListClient) {
				for (Reservation r : c.ListReservation) {
					// si l'id existe déjà alors que c'est la même réservation
					if (r.sejour != null && (r.sejour.id_sejour == id || r == newRes)) {
		        		JOptionPane.showMessageDialog(null, "ID séjour déjà existant ou séjour déjà existant pour cette réservation");
		        		return;
					}
				}
			}
